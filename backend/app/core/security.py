...
def create_password_reset_token(*,user_id:str,organization_id:str,minutes:int=60)->str:
    s=get_settings();now=datetime.now(UTC)
    return jwt.encode(
        {"sub":user_id,"org":organization_id,"type":"password_reset","iat":now,"exp":now+timedelta(minutes=minutes)},
