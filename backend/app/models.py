...
class Opportunity(Base):
    ...
    id:Mapped[uuid.UUID]=mapped_column(primary_key=True,default=uuid.uuid4);organization_id:Mapped[uuid.UUID]=mapped_column(ForeignKey("organizations.id",ondelete="CASCADE"),index=True);company:Mapped[str]=mapped_column(String(160));contact:Mapped[str]=mapped_column(String(160));stage:Mapped[str]=mapped_column(String(40),default="new");value_cents:Mapped[int]=mapped_column(Integer,default=0);created_at:Mapped[datetime]=mapped_column(DateTime(timezone=True),server_default=func.now())
class Agent(Base):
